...
このディレクトリでは、FastAPIとUvicornを利用したAPIを提供します。
APIは、外部システムやアプリケーションがHTTP経由でデータや機能を利用するための入口であり、データ取得、連携、認証付きアクセスの検証などに広く利用されます。

FastAPIはPythonで高速にAPIを構築しやすいWebフレームワークで、OpenAPIによるドキュメント生成やバリデーションを標準で利用できる点が特徴です。
UvicornはそのAPIを実行するASGIサーバーであり、ローカル環境でも軽量にAPIを起動できます。

本リポジトリでは、主に天気情報取得用のサンプルAPIとして利用し、Keycloakによるトークン発行と組み合わせて、認証付きAPI連携の流れを確認できるようにしています。
